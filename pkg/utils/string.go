...
func SplitYamlAndGetListByKind(src []byte) (map[string][][]byte, map[string]string, error) {
	...
	for _, resource := range yamlParts {
		...
		if len(y.Kind) > 0 {
			log.GetLogger().Debugf("group by kind %s resource %s", y.Kind, resource)
			m[y.Kind] = append(m[y.Kind], resource)
			log.GetLogger().Debugf("grouped by kind %s m[y.Kind] %s ", y.Kind, m[y.Kind])
		}
	}
